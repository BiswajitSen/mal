...
const is_macro_call = (ast, env) => {
  ...
  const elt = ast.value[0];
  if (elt instanceof MalSymbol) {

    const fn = env.find(elt) && env.get(elt);
    return (fn instanceof MalFunction) && fn.is_macro;
  }
...
const EVAL = (ast, env) => {
  while (true) {
    ...
    switch (true) {
      ...
      case firstElement === 'defmacro!':
        return handleDef(ast, env, true);
      case firstElement === "macroexpand":
